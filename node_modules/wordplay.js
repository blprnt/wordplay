	...
	if (corpus) {
		...
		for (var i = 0; i < candidates.length; i++) {
			...
			for(var j = 0; j < words.length - matchList.length + 1; j++) {
				var sss = posa.slice(i, i + matchList.length).join(" ");

				//edit distance calculation needs to go here
				var posdist = computeEditDistance(sss, match);
     			var posfdist = posdist / sss.length;
				if (posfdist < tol && stressa.length >= words.length) {


					var seg = words.slice(i, i + matchList.length).join(" ");
					var sstress = stressa.slice(i, i + matchList.length).join(" ");
					var scounts = counta.slice(i, i + matchList.length).join(" ");
					...
					var syllables = countSyllables(sstress);

					var dist = computeEditDistance(stress, sstress);
					float stol = dist/stress.length;

					if((stol < tol || stress.length == sstress.length) && true) {
						console.log(seg);
					}
